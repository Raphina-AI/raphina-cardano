import { Data, generatePrivateKey, Lucid, LucidEvolution } from "@lucid-evolution/lucid"
import { createDiagnosisDatum, decodeDiagnosisDatum, decrypt, encrypt, getPinataSDK, minLoveLaceForDiagnosisDatum, provider } from "../helpers";
import { RedeemerDataType } from "../aiken_types";
...
export const getFilteredDiagnosis = async (req: Request, res: Response) => {
    try {
...
export const getPresignedUrlForThirdPartyFileStorage = async (req: Request, res: Response) => {
    try {
...
export async function storeDiagnosis(req: Request, res: Response) {
    try {
...
async function getBalance(lucid: LucidEvolution) {
    const utxos = await lucid.wallet().getUtxos();
    const balance = utxos.reduce((acc, utxo) => acc + utxo.assets.lovelace, 0n);
    return Number(balance);
}

async function createWallet(lucid: LucidEvolution) {
